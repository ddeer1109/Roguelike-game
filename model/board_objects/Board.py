...
class Board:
    ...
    def generate_room_elements(self):
        ...
        for room in [self.central_room, self.left_room, self.right_room]:
            ...
            for _ in range(random.randint(1,3)): room.create_food()
            for _ in range(random.randint(1,3)): room.create_bandit()
            for _ in range(random.randint(1,3)): room.create_bandit(ArcherBandit)
            for _ in range(random.randint(1,2)): room.create_bandit(MagicianBandit)
